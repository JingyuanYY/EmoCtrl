...
def emotion_score(device):
    from flow_grpo.emo_scorer import EmoScorer

    scorer = EmoScorer(device)

    def _fn(images, prompts, metadatas):
        if not isinstance(images, torch.Tensor):
            images = images.transpose(0, 3, 1, 2)  # NHWC -> NCHW
            images = torch.tensor(images, dtype=torch.uint8) / 255.0
        targets = [metadata.get("emotion") for metadata in metadatas]
        scores = scorer(images, targets)
        return scores, {}
...
def geneval_score(device):
    ...
    def _fn(images, prompts, metadatas, only_strict):
        ...
        if isinstance(images, torch.Tensor):
            ...
            images = images.transpose(0, 2, 3, 1)  # NCHW -> NHWC
        images_batched = np.array_split(images, np.ceil(len(images) / batch_size))
        metadatas_batched = np.array_split(
            metadatas, np.ceil(len(metadatas) / batch_size)
        )
        all_scores = []
        all_rewards = []
        all_strict_rewards = []
        ...
        return (
            ...
            all_rewards,
            all_strict_rewards,
            all_group_rewards_dict,
            all_group_strict_rewards_dict,
        )
...
def main():
    ...
    images = torch.stack(
        [transform(Image.open(image_path).convert("RGB")) for image_path in image_paths]
    )
    prompts = [
