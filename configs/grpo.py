...
def emoset_sdxl_llm():
    """SDXL + 微调 LLM 在线生成 prompt 的 GRPO 训练配置"""
    gpu_number = 8
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "data/emoset")
    config.run_name = "joint_train_emoset_emoimpove_opposite"

    # SDXL base
    config.pretrained.model = "/mnt/d/models/stable-diffusion-xl-base-1.0"
    ...
    config.sample.eval_guidance_scale = 7.5
    ...
    if config.sample.num_batches_per_epoch % 2 != 0:
    ...
    config.train.emoenhance_path = "./results/stage2"

    # 保存与评估频率
    config.save_freq = 60
    config.eval_freq = 60
    config.save_dir = f"logs/emoctrl/{config.run_name}"
    ...
    # ===== 启用 LLM 预生成 prompt =====
    config.use_llm_prompt = True
    config.llm.base_model = "/mnt/d/models/Qwen3-0.6B"
    config.llm.checkpoint = "./results/stage1"
    config.llm.prompt_template = (
        "Based on the <element>, give me a prompt to generate an image."
    )
    config.llm.max_new_tokens = 128
    ...
    config.per_prompt_stat_tracking = True
    # 默认启用断点续练：自动从最新 checkpoint 恢复
    # 设为 None 则从头训练，设为具体路径可从指定 checkpoint 恢复
...
def general_ocr_wan2_1():
    ...
    config.train.gradient_accumulation_steps = (
        config.sample.num_batches_per_epoch * config.sample.sample_time_per_prompt // 2
        if (config.sample.num_batches_per_epoch * config.sample.sample_time_per_prompt)
        > 1
        else 1
    )
    config.train.num_inner_epochs = 1
...
def general_ocr_sd3():
    ...
    assert config.sample.num_batches_per_epoch % 2 == 0, (
        "Please set config.sample.num_batches_per_epoch to an even number! This ensures that config.train.gradient_accumulation_steps = config.sample.num_batches_per_epoch / 2, so that gradients are updated twice per epoch."
    )
...
def pickscore_sd3_fast():
    ...
    config.sample.num_batches_per_epoch = int(
        ...
        / (
            gpu_number
            * config.sample.mini_num_image_per_prompt
            / config.sample.num_image_per_prompt
        )
    )
    config.sample.test_batch_size = 16  # This bs is a special design, the test set has a total of 2048, to make gpu_num*bs*n as close as possible to 2048, because when the number of samples cannot be divided evenly by the number of cards, multi-card will fill the last batch to ensure each card has the same number of samples, affecting gradient synchronization.
